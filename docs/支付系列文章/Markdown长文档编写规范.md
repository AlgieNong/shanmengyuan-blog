...
**设计原则**：
- **简化导航**：只在文章开头提供完整目录，章节内不再重复导航信息
- **选择性使用**：仅在特别长的章节（>2000字）开头提供简要提示
- **分隔线使用**：在主要章节之间使用分隔线，创造视觉节奏
- **避免过度**：不要在每个章节都添加进度提示和当前位置，会显得啰嗦

### 3. 章节总结与过渡（精简版）

长文章需要在章节之间提供总结和过渡，但要简洁：
...
> **💡 思考**：理解了支付后，如果每笔支付都立即结算，会发生什么？这就是我们下一章要讨论的"清算"。

---

## 二、清算的原理

如果每笔支付都立即结算，效率会非常低。我们需要一个机制，把所有交易汇总起来，统一算账。这就是"清算"。
```

**设计原则**：
- **精简小结**：只在核心章节结束时提供简要小结（2-3个要点），不要过于详细
- **简洁过渡**：使用简洁的思考题或疑问句过渡，不超过2句话
- **避免重复**：下一章节开头简要回顾即可，不要重复上一章节的详细内容
- **选择性使用**：不是每个章节都需要小结，只在重要转折点使用
...
长文章中的原理讲解需要由浅入深，但不要机械套用四层结构：

**核心概念**（使用完整结构，但不要标注"第一层"、"第二层"等）：

```markdown
## 清算的原理

想象一下，你和几个朋友一起聚餐，每个人都付了一些钱。最后要 AA 制，大家需要算清楚：总共花了多少钱？每个人应该付多少钱？
...
如果每笔交易都单独结算，效率太低了。通过清算，可以把一天的所有交易汇总起来，一次性算清楚，效率大大提高。
...
**BIN码（Bank Identification Number）** 是银行卡号的前6位，用于识别发卡行。就像手机号的区号，银行卡号的BIN码用于识别发卡银行。

银联根据BIN码查询路由表，将交易请求转发到正确的发卡行。例如，卡号622200开头表示工商银行。
```

**设计原则**：
- **不要标注层次**：不要使用"第一层"、"第二层"等标注，让内容自然流畅
- **灵活选择结构**：核心概念使用完整结构，次要概念使用简化结构
- **避免重复类比**：同一个类比不要重复使用
- **直接进入主题**：不要过度铺垫，直接讲解核心内容
...
**设计原则**：
- **不推荐使用**：进度提示会打断阅读节奏，显得啰嗦
- **仅在必要时使用**：如果文章特别长（>15000字），可以在文章开头提供整体进度提示
- **简化导航**：依赖文章目录和章节标题进行导航，无需额外进度提示
...
### 可读性
- [ ] **核心专业名词首次出现时详细解释**（通俗解释+类比+专业定义+英文对照）
- [ ] **次要专业名词首次出现时简化解释**（通俗解释+专业定义）
- [ ] **没有重复解释**：已解释过的概念后续直接使用，不重复解释
- [ ] **没有重复类比**：同一个类比不在多处重复使用
- [ ] 句子简洁，不超过 20 字
- [ ] 使用具体例子说明抽象概念
- [ ] 使用主动语态
- [ ] 原理讲解由浅入深，但不过度展开
- [ ] **文章长度控制**：8000-12000字，避免过长
...
### 长文章特殊要求
- [ ] 专业名词解释格式统一（核心概念：详细解释；次要概念：简化解释）
- [ ] 历史背景有时间节点标注
- [ ] 原理讲解由浅入深，但**不要标注"第一层"、"第二层"等**
- [ ] 对比内容格式清晰（使用分隔线区分）
- [ ] **不推荐使用进度提示**，避免打断阅读节奏
...
3. **检查专业名词**：确保**所有**专业名词首次出现时详细解释
   - 使用全文搜索功能，查找所有专业名词
   - 检查每个专业名词首次出现的位置
   - 确保每个专业名词都有：通俗解释、类比说明、专业定义、英文对照
